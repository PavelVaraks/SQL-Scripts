...
[void] $MainMirroringListBox.Items.Add("$name is up")
IF ([System.Net.Dns]::GetHostByName($name) |Where-Object Hostname -Like "*idmz*"){
$MainMirroringListBox.ForeColor ='red'
[void] $MainMirroringListBox.Items.Add("Это сервер IDMZ, перезагружаем руками")
$Checkbutton.Enabled = $true
}
ELSE
{
...
{
$MainMirroringListBox.ForeColor ='red'
[void] $MainMirroringListBox.Items.Add("Это не MS SQL Server или служба остановлена")
$Checkbutton.Enabled = $true
Start-Sleep 2
}
Else 
{
$lastbootup=Invoke-Command -ComputerName $name -ScriptBlock {Get-WmiObject win32_operatingsystem | select @{LABEL='LastBootUpTime'
...
[void] $MainMirroringListBox.Items.Add("$a")
...
$RebootServer.Enabled = $true
[void] $MainMirroringListBox.Items.Add($RebootPending)
...
[void] $MainMirroringListBox.Items.Add("Cервер $name - Зеркало,можно перезагрузить в рабочее время!")
$Checkbutton.Enabled=$true
...
{
$MainMirroringListBox.ForeColor ='red'
[void] $MainMirroringListBox.Items.Add("Это основной сервер или зеркалирование не настроено")
$CloseWindowButton.Enabled = $true
$RebootServer.Enabled = $false
$Checkbutton.Enabled = $true
}
...
[void] $MainMirroringListBox.Items.Add("$name is down")
$Checkbutton.Enabled = $true
}
}
     
...
$MirrorSRVName=$MirrorSRV.Column1
IF ($MirrorSRVName -eq $name){
[void] $MainMirroringListBox.Items.Add("Останавливаем Репликацию на сервере $PrincipalMirrorName")
...
[void] $MainMirroringListBox.Items.Add("Репликация остановленна")}

Invoke-Command -ComputerName $name -ScriptBlock{
Stop-Service  -name MSSQLSERVER  -Force }
Start-Sleep 5
Restart-Computer -ComputerName $name -Force
[void] $MainMirroringListBox.Items.Add("Сервер $name отправлен в перезагрузку")
DO
{
if
(Test-Connection -ComputerName $name -Count 1 -ErrorAction SilentlyContinue)
{$MainMirroringListBox.Items.Add("Сервер $name доступен")
$MSSQL=Get-Service -Name MSSQLSERVER -ComputerName $name
IF
($MSSQL.status -eq "Running")
{
$MainMirroringListBox.Items.Add("Служба запущена")
Start-Sleep 10
$MainMirroringListBox.Items.Add("Восстанавливаем репликацию зеркало")
Invoke-Sqlcmd2 -ServerInstance $PrincipalMirrorName -Query $resumeMirror
$MirrorStatus=Invoke-Sqlcmd2 -ServerInstance $PrincipalMirrorName -Query $SqlMirrorStatus
$MirrorStatusInt=$MirrorStatus.Column1
IF ($MirrorStatusInt -eq 0)
{
$MainMirroringListBox.Items.Add("Зеркало Восстановлено")
Invoke-Sqlcmd2 -ServerInstance $PrincipalMirrorName -Query $resumeMirror
$RebootServer.Enabled = $false
}
}
...
$MainMirroringListBox.Items.Add("$time")
$MainMirroringListBox.Items.Add("Служба не запущена")
Start-Sleep 10
$MainMirroringListBox.Items.Clear()
}
}
ELSE
{$MainMirroringListBox.Items.Add("Сервер недоступен, через 10 секунд опросим снова")
Start-Sleep 10
}
}
...
$MainMirroringListBox.Items.Clear()
$name=$ServerNameBox.Text
[void] $MainMirroringListBox.Items.Add("Мы проверяли $checkedName, а хочешь перезагрузить $name - Ай-ай!")
$CloseWindowButton.Enabled = $true
$Checkbutton.Enabled = $true
}
}     
...
$MainMirroringListBox = New-Object System.Windows.Forms.ListBox 
