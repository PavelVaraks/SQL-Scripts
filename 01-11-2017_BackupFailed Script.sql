...
exec msdb.dbo.sp_delete_alert @name=N'18210 Backup Failed'
GO
...
set @servername = (Select CONVERT(nvarchar,SERVERPROPERTY('Servername'),2))
...
set @time1 =convert(VARCHAR(30), getdate()-0.00005578703300, 21)
set @time2 =convert(VARCHAR(30), getdate()+0.00005578703300, 21)
...
set @subject2 = (select convert(nvarchar,serverproperty(''Servername''),2))
set @subject5 =
''_''+(select top 1 CASE 
		...
		WHEN TEXT LIKE ''%BACKUP LOG%''
			THEN ''Log''
			WHEN TEXT LIKE ''%BACKUP DEVICE%''
			THEN ''Backup''
		END
		from  #BackupError
		where TEXT like ''%DIFFERENTIAL%'' or
		text like ''%BACKUP DATABASE%'' or TEXT like ''%BACKUP LOG%'' or TEXT like ''%BACKUP DEVICE%'')
...
select job_id from msdb.dbo.sysjobs
where name = 'ErrorBackupMailSend')
EXEC msdb.dbo.sp_add_alert @name=N'18210 Backup Failed', 
		@message_id=18210, 
		@severity=0, 
		@enabled=1, 
		...
		@job_id=@jobID
GO
EXEC msdb.dbo.sp_add_notification @alert_name=N'18210 Backup Failed', @operator_name=N'AlertSql', @notification_method = 1
GO
